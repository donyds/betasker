...
if(isset($_GET['task_id'])){
    ...
    $acc           = mysqli_escape_string($con,$_GET['acc']);
    $status        = mysqli_escape_string($con,$_GET['status']);
     
    $getposterid = mysqli_query($con,"SELECT * FROM task_award WHERE task_receiver='$acc' AND task_id='$task_id'");
        while($get_getposterid = mysqli_fetch_array($getposterid)){
          
            $poster_id  = $get_getposterid['task_creator'];
            $task_awd   = $get_getposterid['status'];
            ...
            $getpostername = mysqli_query($con,"SELECT * FROM account WHERE account_id='$poster_id'");
            while($postername = mysqli_fetch_array($getpostername)){
            
                $post_name = $postername['fullname'];
                $email_address = $postername['email_address'];
    ...
    if($status == '1'){
        ...
        $post_stats ="Approved";
        $results1 = mysqli_query($con,"UPDATE task_award SET status='$status' WHERE task_receiver='$acc' AND task_id='$task_id'");
        $results2 = mysqli_query($con,"UPDATE task_apply SET status='$status' WHERE account_id='$acc' AND task_id='$task_id'");
        
        
    }else{
        $post_stats ="Decline";

        $results1 = mysqli_query($con,"UPDATE task_award SET status='2' WHERE task_receiver='$acc' AND task_id='$task_id'");
        $results2 = mysqli_query($con,"UPDATE task_apply SET status='2' WHERE account_id='$acc' AND task_id='$task_id'");
...
$mail->Subject = "Notification For Proposal";
...
<p style='font-size: 14px; line-height: 160%;padding-top:10px;'><span style='font-size: 22px; line-height: 35.2px;'>Hi $post_name, </span></p>
<p style='font-size: 14px; line-height: 160%;'><span style='font-size: 18px; line-height: 28.8px;'>  Your project is $post_stats  from freelancer.<br> </span></p>
...
// mail function end
    ...
    header('Location : ../../approve_from_receiver.php');

    }else{

    header('Location: ../../reject_from_receiver.php');
