...
if(isset($_GET['id']) && isset($_GET['chat'])){
...
$chatsid  = mysqli_escape_string($con,$_GET['chat']);
//get request data
$getreq  = mysqli_query($con,"SELECT * FROM chat_req_pay WHERE id='$id' ");
$get_req = mysqli_fetch_array($getreq);
//end
$amount   = $get_req['amount']; 
$currency = $get_req['currency']; 
$task_id  = $get_req['task_id']; 
$task_creator   = $get_req['task_creator']; 
$task_receiver  = $get_req['task_receiver']; 
...
$getu1     = mysqli_query($con,"SELECT * FROM account WHERE account_id='$task_receiver'");
...
if($amount > $wallet){
header('Location: ../../req_list_creator.php?chat='.$chatsid.'&task_id='.$task_id.'&en');

}else{

$sqls    = "UPDATE chat_req_pay SET status='1',title='Paid' WHERE id='$id'";
$results = mysqli_query($con,$sqls);
$stamp      = date('Y-m-d H:i:s');
$uname    = $_COOKIE['username']; 
$message  = $uname." Has  €".$amount." Amount  Request Approved";

      
$chat = mysqli_query($con,"INSERT INTO `chat_message` (`timestamp`, `room_id`, `account_id`, `role`, `message`, `img`, `status`) VALUES ('$stamp', '$chatsid', '$account_id', 'user', '$message', '0', '0')"); 



// add here all transaction
$sql    = mysqli_query($con,"INSERT INTO payment_transaction(date,time,title,amount,transaction_id,account_from,account_to,task_id,to_name,status) VALUES('$date','$time','$title','$amount','$transaction_id','$account_id','$task_receiver','$task_id','$to_name','accept')");

$update = mysqli_query($con,"UPDATE account SET wallet='$total_bal' WHERE account_id='$account_id'");

$update2 = mysqli_query($con,"UPDATE account SET wallet='$total_bal2' WHERE account_id='$task_receiver'");
...
header('Location: ../../view-chat.php?room_id='.$chatsid);
