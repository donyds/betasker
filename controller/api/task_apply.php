...
$app           = mysqli_escape_string($con,$_POST['app']);

$account_id    = $_COOKIE['id'];

$proposal_id   = uniqid();

$task_proposal = mysqli_escape_string($con,$_POST['task_descp']);

$budget        = mysqli_escape_string($con,$_POST['task_budget']);


$chk_task = mysqli_query($con,"SELECT * FROM task_apply WHERE task_id='$task_id' AND account_id='$account_id'");
$get_coun = mysqli_num_rows($chk_task);

if($get_coun > 0){

header('Location: ../../make-an-offer.php?task_id='.$task_id.'&already');

}else{

    $sql = mysqli_query($con,"INSERT INTO task_apply(task_id,account_id,date,time,proposal_id,task_proposal,budget) VALUES('$task_id','$account_id','$date','$time','$proposal_id','$task_proposal','$budget')");


    $sql_task = mysqli_query($con,"SELECT * FROM task_create WHERE task_id='$task_id'");
    $get_task = mysqli_fetch_array($sql_task);

    $creator        = $get_task['account_id'];
    $task_title     = $get_task['task_title'];
     ...
     //get user details
    $udetails = mysqli_query($con,"SELECT * FROM account WHERE account_id ='$account_id'");
    ...
    $username  = $get_u['fullname'];
    $email     = $get_u['email_address'];

    //for notification only

    $ad1  = "<b>".$fname."</b>. has made an offer on <b>". $task_title ."</b>" ;
    $ad2  = $creator;
    $ad3  = $task_id;
    $send = mysqli_query($con,"INSERT INTO `notification` (`date`, `title`, `task_creator`, `task_id`) VALUES ('$min_ago', '$ad1', '$ad2', '$ad3')");
                                                    ...
                                                    <tr>
                                                        <td style='overflow-wrap:break-word;word-break:break-word;padding:33px 55px;font-family:'Cabin',sans-serif;' align='left'>
                                                            ...
                                                            <div style='color: #000000;background-color:#e8faff;line-height: 160%; text-align: center; word-wrap: break-word;'>
                                                                ...
                                                                <p style='font-size: 14px; line-height: 160%;'><span style='font-size: 18px; line-height: 28.8px;'>  Someone is sent you proposal request. <br> </span></p>
