<?php
     ...
     $doubt_id=htmlspecialchars($_GET['room_id']);
     $uid = $_COOKIE['id'];

     //get task id 
     $get_room_iddata = mysqli_query($con,"SELECT * FROM `chat_room` where room_id = '$doubt_id'");
     ...
     $ad_fiftenday     = date('Y-m-d', strtotime($getclose_date. ' + 15 days'));
     $close_date_from  = date("Y-m-d");


     $sql_doubt= mysqli_query($con,"SELECT * FROM `chat_message` where room_id = '$doubt_id' ORDER BY id ASC");
